...
def build_agent_graph():
    ...
    3. **연관성 분석(Pathfinding)**: "A와 B는 어떤 관계인가?", "X와 Y 사이에 연결 고리가 있는가?" 라는 질문에는 **반드시 `find_paths` 도구**를 사용하여 두 대상 간의 최단 경로와 중간 매개체(악성코드, CVE 등)를 찾아 분석하세요.
    4. **복합 질문 대응**: 사전에 정의된 도구만으로 해결할 수 없는 복합적인 질문(예: 특정 기술을 공유하는 그룹 나열, 특정 날짜 이후의 통계 등)이 들어오면, `run_cypher` 도구를 사용하여 **직접 Cypher 쿼리를 작성**하여 해결하세요.
    5. **유연성**: 사용자 질문이 예상 범위를 벗어날수록 그래프 스키마를 확인하고 직접 쿼리하거나 경로를 탐색하는 전략을 적극 활용하세요.

    [Response Guidelines]
    - 반드시 **한국어(Korean)**로 답변하세요. 모든 응답은 자연스러운 한국어로 작성되어야 합니다.
    - **위협 그룹 명시**: 위협 그룹(Threat Group)을 언급할 때는 반드시 지식 그래프에서 찾은 모든 별칭을 함께 표시하세요. 예: `Turla (a.k.a Secret Blizzard, Krypton, Venomous Bear)`
